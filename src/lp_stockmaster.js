...
let options;
const argsSchema = [
    ['l', false], // Stop any other running lp_stockmaster.js instances and sell all stocks
    ...
    ['reserve', 10e6], // A fixed amount of money to not spend. Default 10mil
    ...
    ['buy-threshold', 0.001], // Buy only stocks forecasted to earn better than a 0.1% return (1 Basis Point)
    ['sell-threshold', 0], // Sell stocks forecasted to earn less than this return (default 0% - which happens when prob hits 50% or worse)
    ['diversification', 1], // Before we have 4S data, we will not hold more than this fraction of our portfolio as a single stock
    ...
    ['pre-4s-forecast-window', 76], // This much history will be used to determine the historical probability of the stock (so long as no inversions are detected) (Default 76)
    ...
    ['pre-4s-minimum-hold-time', 8], // A recently bought position must be held for this long before selling, to avoid rash decisions due to noise after a fresh market cycle. (Default 10)
    ['buy-4s-budget', 0.8], // Maximum corpus value we will sacrifice in order to buy 4S. Setting to 0 will never buy 4s. Default 0.8
];
...
export async function main(ns) {
    ...
    const hasTixApiAccess = await getNsDataThroughFile(ns, 'ns.stock.hasTIXAPIAccess()', '/StockTemp/hasTIX.txt');
    if (runOptions.l || runOptions.liquidate) {
        ...
        await runCommand(ns, `ns.ps().filter(proc => proc.filename == '${ns.getScriptName()}' && !proc.args.includes('-l') && !proc.args.includes('--liquidate'))` +
            `.forEach(proc => ns.kill(proc.pid))`, '/StockTemp/kill-stockmarket-scripts.js');
    ...
    if (!hasTixApiAccess) { // You cannot use the stockmaster until you have API access
        ...
        do {
            ...
            try {
                ...
                log(ns, `WARNING: lp_stockmaster.js Caught (and suppressed) an unexpected error while waiting to buy stock market access:\n` +
...
async function getPlayerInfo(ns) {
    return await getNsDataThroughFile(ns, `ns.getPlayer()`, '/StockTemp/player-info.txt');
}
...
async function getStockInfoDict(ns, stockFunction) {
    ...
    return await getNsDataThroughFile(ns,
        ...
        `/StockTemp/stock-${stockFunction}.txt`, allStockSymbols);
};
...
async function updateForecast(ns, allStocks, has4s) {
    ...
    await ns.write('/StockTemp/stock-probabilities.txt', JSON.stringify(Object.fromEntries(
...
let summaryFile = '/StockTemp/stockmarket-summary.txt';
...
let launchSummaryTail = async ns => {
    ...
    if (await getNsDataThroughFile(ns, `ns.scriptRunning('${summaryTailScript}', ns.getHostname())`, '/StockTemp/stockmarket-summary-is-running.txt'))
        return;
...
let transactStock = async (ns, sym, numShares, action) =>
    await getNsDataThroughFile(ns, `ns.stock.${action}(ns.args[0], ns.args[1])`, `/StockTemp/stock-${action}.txt`, [sym, numShares]);
...
function doStatusUpdate(ns, stocks, myStocks, disableHud) {
...
async function checkAccess(ns, stockFn) {
    return await getNsDataThroughFile(ns, `ns.stock.${stockFn}()`, `/StockTemp/stock-${stockFn}.txt`)
}
