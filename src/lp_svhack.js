...
export async function main(ns) {
	...
	// Declare vars & consts
	let targetCoef = 0.5;
	let allServs = nstb.ScanAll(ns, true)[0];
	let allTargetServs;
	let allUsableServs = FindAllUsable(allServs);
	let homeSize;
	const hackSize = await nstb.RunCom(ns, 'ns.getScriptRam()', ['smart-hack.js']);
	const chargeSize = await nstb.RunCom(ns, 'ns.getScriptRam()', ['smart-charge.js']);

	// Settings
	const hackInfluencesStocks = false
	const growInfluencesStocks = true
	
	KillEverything(allUsableServs);

	var beingHacked = {};

	while (true) { await HackLoop() }
	...
	async function HackLoop() {
		...
		homeSize = await nstb.RunCom(ns, 'ns.getServerMaxRam()', ['home']);
		
		// Figure out what fraction of our home RAM should be left unused in order to run core scripts
		let coreCoef = 0.5;
		if (homeSize <= 2048) coreCoef = 1;
		let maxRamForCore = homeSize * coreCoef;

		// Decrement all second (tick) counts for servers in beingHacked, removing them from the dict if their ticks value is below 0
		// (as this means the hack/weak/grow done to this server is finished).
		for (let key in beingHacked) beingHacked[key][0] -= 1;
